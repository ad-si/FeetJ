...
MTQ_QML_REGISTER_PLUGIN(FjSlider)

FjSlider::FjSlider(QQuickItem *parent)
	: BaseWidget(parent),
	  m_active(false),
	  m_value(0.5),
...
void FjSlider::paint(QPainter *painter)
{
...
void FjSlider::processTapDown(mtq::TapEvent *event)
{
...
void FjSlider::processTapUp(mtq::TapEvent *event)
{
	setActive(false);
}

void FjSlider::processTapCancel(mtq::TapEvent *event)
{
	setActive(false);
}

void FjSlider::setValueByTapX(int x) {
...
qreal FjSlider::value() const
{
	return m_value;
}

void FjSlider::setValue(const qreal value)
{
	m_value = value;
}

bool FjSlider::active() const
{
	return m_active;
}

void FjSlider::setActive(const bool state)
{
	m_active = state;
	update();
}
